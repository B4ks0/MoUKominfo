...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{waterfall.jpg}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{usecase_diagram_new.jpg}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{activity_diagram_new.jpg}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{architecture_diagram.jpg}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{database_erd.jpg}
\caption{Entity Relationship Diagram (ERD) Database}
\label{fig:erd}
\end{figure}

\subsection{Analisis Kinerja dan Efektivitas Sistem}
Evaluasi kinerja sistem dilakukan dengan membandingkan efisiensi operasional antara metode manual dan sistem digital yang dikembangkan. Berdasarkan pengujian paired t-test, ditemukan perbedaan yang sangat signifikan (t(49)=26.84, p<0.001, Cohen's d=3.79), yang mengindikasikan peningkatan efisiensi yang substansial. Gambar \ref{fig:performance} memvisualisasikan tren penurunan waktu pemrosesan yang drastis setelah penerapan sistem digital.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{performance_comparison.jpg}
\caption{Perbandingan Waktu Pemrosesan Manual vs Digital}
\label{fig:performance}
\end{figure}

Secara spesifik, Tabel \ref{tab:processing_time} memperlihatkan bahwa rata-rata waktu pemrosesan berkurang secara drastis dari 42.5 menit menjadi hanya 6.3 menit, mencapai reduksi waktu sebesar 85.2\%.
...
Selain kecepatan, akurasi data juga mengalami peningkatan signifikan. Seperti ditunjukkan pada Tabel \ref{tab:accuracy}, tingkat akurasi meningkat dari 81.7\% pada sistem manual menjadi 97.5\% pada sistem digital, sementara tingkat kesalahan (error rate) menurun dari 18.3\% menjadi 2.5\%.
...
Pengujian teknis lebih lanjut menunjukkan bahwa sistem memiliki performa yang handal dengan \textit{average response time} 1.4 detik, \textit{page load time} 2.5 detik, \textit{throughput} 115 pendaftaran per hari, dan \textit{system uptime} mencapai 99.2\%.

\subsection{Evaluasi Usabilitas dan Dampak Organisasi}
Penerimaan pengguna terhadap sistem diukur menggunakan System Usability Scale (SUS) dengan 42 responden. Hasil analisis menunjukkan skor rata-rata (mean) sebesar 76.8, median 78.0, dan standar deviasi 9.2 dengan rentang 55.0-90.0, yang menempatkan sistem pada kategori Grade B (Good). Distribusi skor SUS dapat dilihat pada Gambar \ref{fig:sus_dist}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{sus_distribution.jpg}
\caption{Distribusi Skor System Usability Scale}
\label{fig:sus_dist}
\end{figure}

Dampak implementasi sistem terhadap organisasi sangat positif, seperti yang dirangkum dalam Tabel \ref{tab:organizational}. Pendaftaran per bulan meningkat dari 78 menjadi 92, jam kerja admin per bulan berkurang drastis dari 165 menjadi 28 jam, dan keluhan per bulan menurun dari 11 menjadi 2.
...
Antarmuka sistem dirancang dengan mengutamakan pengalaman pengguna (User Experience), memastikan kemudahan akses dan navigasi. Gambar \ref{fig:halaman_daftar} menampilkan halaman pendaftaran wartawan yang responsif, sedangkan Gambar \ref{fig:halaman_admin1} menampilkan dashboard admin untuk manajemen data.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{wartawan.jpg}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{admin1.jpg}
\caption{Halaman Dashboard Admin}
\label{fig:halaman_admin1}
\end{figure}

Kualitas fungsional sistem divalidasi melalui pengujian Black Box Testing, yang memastikan setiap fitur dieksekusi sesuai spesifikasi. Hasil pengujian pada Tabel \ref{tab:blackbox} memperlihatkan bahwa seluruh fungsi utama—mulai dari halaman utama, pendaftaran, cek status, login admin, hingga verifikasi—berjalan sesuai harapan tanpa kesalahan.
